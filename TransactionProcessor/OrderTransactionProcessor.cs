...
namespace ScheduleD
{
    public class OrderTransactionProcessor
    {
        ...
        public IList<MatchedTrade> WashSales { get; set; }
        ...
        public OrderTransactionProcessor()
        {
            ...
            WashSales = new List<MatchedTrade>();
        }

        public void ProcessTransaction(OrderTransaction trans)
        {
            //string comment;
            // special case where option is assigned.  
            //   Do nothing because the sale/purchase will be handled in a regular transactions
            ...
            switch (trans.Direction)
            {
                case OrderDirection.Buy:
                    if (position.SellsCount() > 0)
                    {
                        ...
                        // If the option was assigned, the sale/purchase is in a separate record
                        //   So we just want to match the sale of the call with a trans with 0 cost.
                        if (trans.ActionId == 52)
                            trans.Quantity = selltrans.Quantity * -1;
                ...
                case OrderDirection.Sell:
                    if (position.BuysCount() > 0)
                    {
                        ...
                        if (trans.ActionId == 52)
                            trans.Quantity = buytrans.Quantity * -1;
